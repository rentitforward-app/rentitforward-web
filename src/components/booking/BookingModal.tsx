...
export function BookingModal({ isOpen, onClose, listing, user }: BookingModalProps) {
  ...
  const onSubmitBooking = async (data: BookingForm) => {
    ...
    try {
      ...
      // Use the backend API to create booking with proper payment calculation
      const bookingRequestData = {
        ...
        delivery_fee: deliveryFee, // Explicitly pass the delivery fee
        pickup_instructions: data.notes || null,
        include_insurance: includeInsurance,
        points_to_redeem: 0, // For now, no points redemption in this flow
      };
      
      console.log('Creating booking via API with data:', bookingRequestData);
      ...
      console.log('Stored total_amount:', bookingData.total_amount);
      console.log('Expected total_amount:', totalAmount);
